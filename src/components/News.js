...
export class News extends Component {
  ...
  async componentDidMount(){
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=57f315a3218d4cdb94a940d1feeb389d&pageSize=${this.props.pageSize}`;
  ...
  handlePreButton = async ()=>{
    let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=57f315a3218d4cdb94a940d1feeb389d&page=${this.state.page -1}&pageSize=${this.props.pageSize}`;
    this.setState({loading : true})
    let data = await fetch(url);
    let parsedData = await data.json()
    this.setState({
  ...
  render() {
    return (
      <div className="container my-4">
        <h1 className="text-center" style={{marginTop : "50px",fontSize:"35px"}}>NewsMonkey - Top Headlines</h1>
